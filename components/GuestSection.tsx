        ...
        <div className="grid grid-cols-3 gap-4 max-lg:grid-cols-1">
          {guests.map((guest) => (
            <article className="reveal relative overflow-hidden border border-ink/20 bg-white max-lg:grid max-lg:grid-cols-[280px_1fr] max-md:block" key={guest.name}>
              <div className="relative aspect-[.95] overflow-hidden bg-white max-lg:row-span-2 max-lg:min-h-85 max-lg:aspect-auto max-md:min-h-0 max-md:aspect-square"><Image className="object-cover object-top" src={guest.image} alt={`${guest.name}さんの写真`} fill sizes="(max-width: 700px) 100vw, 280px" /></div>
